...
const Discussions = (
  ...
  const queryKey = `discussions/${query.category}`
  ...
  const {
    ...
    initialPage,
    token,
    endpoint: `/api/discussion_categories/${query.category}/discussions`,
    queryKey,
    initialData
  })
  ...
  return (<>
    ...
    <ContentLayout sideMenu={currentWorkspace && <DiscussionsLeftMenu workspace={currentWorkspace} />}>
      ...
      {currentWorkspace && can(currentUser, 'discussion.create', currentWorkspace) && <InlineDiscussionForm discussion={{ discussion_category: category } as unknown as Discussion} workspace={currentWorkspace} />}
