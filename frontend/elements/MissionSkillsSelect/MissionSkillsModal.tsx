...
const MissionSkillsModal = ({
  ...
  const [mandatory, setSkillMandatory] = useState<boolean>(true)
  ...
  const options = data.map((d: {id: number, name: string}) =>
    <Option value={d.id} key={d.name}>{d.name}</Option>
  );
    ...
    <>
      ...
      >
        ...
        {skillCategory && <>
          ...
          {skill && <Form.Item style={{ marginTop: 20 }} name="mandatory" label={`${t('form.skill.mandatory')}`}>
            <Switch
              onChange={setSkillMandatory}
              checked={mandatory}
              checkedChildren={t('form.skill.yes')}
              unCheckedChildren={t('form.skill.no')}
            />
          </Form.Item>}
        </>}
      </Modal>
...
export default MissionSkillsModal